...
Constant hook handlers can also have string values replaced with file contents using `$file:path`, where `path` is the path to the file relative to the directory for this plugin. `${PLUGIN_DIR}` will also be replaced with the directory for this plugin.
