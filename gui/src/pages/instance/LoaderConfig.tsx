...
export default function LoaderConfig(props: LoaderConfigProps) {
	...
	let [supportedLoaderVersions, setSupportedLoaderVersions] = createSignal<{
	...
	let [supportedLoaderVersionsResource, _] = createResource(
		() => {
			...
			let vers =
				props.minecraftVersion == undefined ? "" : props.minecraftVersion;
		...
		async () => {
			...
			if (supportedLoaderVersions()[loader] == undefined) {
				try {
					let versions: string[] = await invoke("get_loader_versions", {
						loader: loader,
	...
	let showClientLoaderVersion = () =>
		props.clientLoader != undefined ||
		props.parentConfigs.some(
			(x) => getConfiguredLoader(x.loader, "client") != undefined
		);
	let showServerLoaderVersion = () =>
		...
		<>
			...
			>
				...
				>
					<InlineSelect
						...
						options={props
							.supportedLoaders!.filter((x) => getLoaderSide(x) != "server")
							.map((x) => {
								return {
									value: x,
									contents: (
										...
										>
											<Show when={x != undefined}>
												<img
													src={getLoaderImage(x as Loader)}
													style="width:1.2rem"
												/>
											</Show>
											{x == undefined
									...
									tip: x == undefined ? "Inherit from the template" : undefined,
								};
							})}
						columns={4}
						allowEmpty={false}
			...
			>
				...
				>
					<InlineSelect
						...
						options={props
							.supportedLoaders!.filter((x) => getLoaderSide(x) != "client")
							.map((x) => {
								return {
									value: x,
									contents: (
			...
			<Switch>
				<Match when={props.minecraftVersion == undefined}>
					<Show when={showClientLoaderVersion() || showServerLoaderVersion()}>
						<div class="cont start label">LOADER VERSION</div>
						<div style="color:var(--fg3)">Select a Minecraft version first</div>
					</Show>
				</Match>
