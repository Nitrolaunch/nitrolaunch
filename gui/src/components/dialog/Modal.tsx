	...
	let buttons = (
		<For each={props.buttons}>
			{(button, i) => {
				...
				return (
					<div
						class={`cont modal-button ${i() == 0 ? "first" : ""} ${
							i() == props.buttons.length - 1 ? "last" : ""
						}`}
	...
	return (
		...
		>
			<div class="modal-contents">
				...
				<div
					class="cont col start modal-body"
					style={`height:${height};${
				...
				<Show when={props.buttons.length > 0}>
					<div
						class="modal-buttons"
						style={`grid-template-columns:repeat(${props.buttons.length}, 1fr)`}
					>
						{buttons}
					</div>
				</Show>
