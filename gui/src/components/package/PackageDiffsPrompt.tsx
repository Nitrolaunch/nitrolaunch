	...
	return (
		...
		>
			<For each={props.diffs}>
				{(diff) => {
					let pkg =
						diff.type == "version_changed"
							? diff.data[0]
							: diff.type.includes("many")
								? undefined
								: diff.data;
					...
					let pkgElement = () =>
						meta() == undefined ? (
							...
							<div class="cont package-diff-package">
								<img
					...
					return (
						<div class="cont package-diff">
							<div class={`cont package-diff-indicator ${diff.type}`}>
								<Switch>
									<Match
										when={diff.type == "added" || diff.type == "many_added"}
									>
										<Icon icon={Plus} size="1rem" />
									</Match>
									<Match
										when={diff.type == "removed" || diff.type == "many_removed"}
									>
							...
							<div class="cont package-diff-info">
								<Switch>
									<Match when={diff.type == "version_changed"}>
										{pkgElement()}
										<span
											class="cont package-diff-version"
									...
									<Match
										when={
											diff.type == "many_added" || diff.type == "many_removed"
										}
									>
										{diff.data} packages
									</Match>
									<Match when={diff.type == "added" || diff.type == "removed"}>
										{pkgElement()}
									</Match>
								</Switch>
							</div>
